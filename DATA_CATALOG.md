...
| `player_forecasts` | ~561/run | `run_id, ffa_id, pos` | previsão probabilística por jogador. Chaves + `season, week, tag, espn_id` (do crosswalk, nullable) + do consenso: `projection` (média das fontes), `n_sources`, `coverage_class`, `source_sd`, `source_mad` + da simulação Monte Carlo: `sim_mean`, `sim_sd`, `p05/p10/p25/p50/p75/p90/p95`, `prob_gt_10/15/20/25/30` + diagnóstico do pool de resíduos: `residual_pool_level` (1--4, ver spec §9.1), `residual_pool_n` + `is_realized` (bool, spec §11.3): jogador cujo jogo já travou --- `draws` é constante nos pontos reais, `sim_sd == 0`, pool de resíduos `NA`. **`sim_mean != projection`** de propósito: a diferença é o viés histórico do consenso |
...
| `player_id` (espn) | id de jogador do ESPN; ponte futura: `ffa_player_ids$espn_id` (D/ST usa offset 60000 --- ver `archive/R_old/import/espn_scraper.R`) |
